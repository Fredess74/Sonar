  ...
  return (
    ...
    >
      ...
      <div className={`flex-1 overflow-y-auto overflow-x-hidden p-4 space-y-3 scrollbar-hide transition-opacity duration-300 ${isCollapsed ? 'opacity-0 hidden' : 'opacity-100'}`}>
         ...
         {selectedStep && selectedStep.place_card ? (
             <div className="animate-fade-in">
                 <button
                    onClick={() => onStepClick('')}
                    className="mb-4 text-sm text-sonar-muted hover:text-white flex items-center gap-1 transition-colors hover:translate-x-[-2px] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sonar-accent rounded px-1"
                 >
      ...
      <div className={`p-4 border-t border-white/5 bg-sonar-bg/50 ${isCollapsed ? 'hidden' : 'block'}`}>
        <button className="w-full py-3 rounded-xl bg-sonar-accent text-sonar-bg font-bold text-sm shadow-neon hover:shadow-neon-strong transition-all hover:-translate-y-0.5 flex items-center justify-center gap-2 group focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-offset-2 focus-visible:ring-offset-sonar-bg">
