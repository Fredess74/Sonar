...
export const BottomSheet = ({ route, onStepClick, selectedStepId }: BottomSheetProps) => {
  ...
  // Derived state pattern to resolve react-hooks/set-state-in-effect warning
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                   <div className="flex justify-between items-start">
                       ...
                       {wp.arrival_time && (
                         <span className="text-xs text-sonar-muted bg-white/5 px-1.5 py-0.5 rounded shrink-0" aria-label={`Arrival time: ${wp.arrival_time}`}>
                           {wp.arrival_time}
                         </span>
                       )}
                   </div>
